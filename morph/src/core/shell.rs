    ...
    S: IntoStyle,
{
    ...
    pub fn size(&self) -> (u32, u32) {
        if let Some(display) = &self.draw_target {
    ...
    /// Sets the display to draw on.
    pub fn display(mut self, display: D) -> Self {
        self.draw_target = Some(display);
        self
    }

    /// Gets a reference to the display.
    pub fn display_ref(&self) -> Option<&D> {
        if let Some(display) = &self.draw_target {
            return Some(display);
        }
    ...
    /// Gets a mutable reference to the display.
    pub fn display_mut(&mut self) -> Option<&mut D> {
        if let Some(display) = &mut self.draw_target {
            return Some(display);
        }
    ...
    fn int_layout(
        ...
        if let Some(widget) = self.context.get(id) {
            let node = self
                ...
                .map_err(|_| MorphError::OutOfBounds("Cannot generate stretch node."))?;
            return nodes
                .push((id, node))
                .map_err(|_| MorphError::OutOfBounds("Cannot add more nodes to layout."));
        }

        Err(MorphError::Other("Cannot initialize layout."))
    }
            ...
            if nodes.len() > 0 {
                if let Some(node) = nodes.get(nodes.len() - 1) {
                    self.stretch
                        ...
                        .map_err(|_| MorphError::Layout("Cannot compute layout."))?;
                }
            }
    ...
    fn int_draw(&mut self, id: WidgetId) -> MorphResult<()> {
            ...
            if let Some(widget) = self.context.get(id) {
                ...
                if let Some(node) = widget.node {
                    let layout = self
                        .stretch
                        .layout(node)
                        .map_err(|_| MorphError::Layout("Cannot get layout of node."))?;
                        ...
                        match widget.drawables.get(i).unwrap().clone() {
                            crate::core::Drawable::Rectangle => {
                                ...
                                if let Some(style) = style {
                                    ...
                                    if let Some(border_color) = style.border_color {
                                        style_builder = style_builder.stroke_color(C::from(
                                            C::Raw::from_u32(border_color.data),
                                        ));
                                    }
                                ...
                                rectangle
                                    ...
                                    .draw(display)
                                    .map_err(|_| {
                                        MorphError::DrawTarget("Cannot draw rectangle.")
                                    })?;
                            }
                            crate::core::Drawable::Line => {
                                ...
                                line.into_styled(PrimitiveStyleBuilder::new().build())
                                    .draw(display)
                                    .map_err(|_| MorphError::DrawTarget("Cannot draw line."))?;
                            }
                            crate::core::Drawable::Circle => {
                                ...
                                circle
                                    ...
                                    .draw(display)
                                    .map_err(|_| MorphError::DrawTarget("Cannot draw circle."))?;
                            }
                            crate::core::Drawable::Triangle => {
                                ...
                                triangle
                                    .into_styled(PrimitiveStyleBuilder::new().build())
                                    .draw(display)
                                    .map_err(|_| MorphError::DrawTarget("Cannot draw triangle."))?;
                            }
                            crate::core::Drawable::Text => {
                                if let Some(text) = widget.text {
                                    ...
                                    text.into_styled(style_builder.build())
                                        .draw(display)
                                        .map_err(|_| MorphError::DrawTarget("Cannot draw text."))?;
                                }
                            }
                            crate::core::Drawable::Image => {
                                ...
                                let image: Image<_, C> = Image::new(
                                    &image_raw,
                                    Point::new(layout.location.x as i32, layout.location.y as i32),
                                );
                                image
                                    .draw(display)
                                    .map_err(|_| MorphError::DrawTarget("Cannot draw image."))?;
                            }
                        }
                    }
                }
